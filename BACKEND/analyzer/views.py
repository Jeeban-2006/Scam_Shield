...
from .utils import analyze_message, send_scam_report_email, _send_to_mongo_async
...
def scam_analyzer(request):
    ...
    # Store in MongoDB Service
    _send_to_mongo_async("messages", {
        "message": text,
        "analysis": result,
        "user": user.email if (user and user.email) else "anonymous",
...
def report_scam(request):
    ...
    try:
        ...
        # Store in MongoDB Service
        _send_to_mongo_async("reports", {
            "reportType": scam_type,
            "description": scam_content_sanitized,
            "evidence": screenshot_path or f"Platform: {platform}",
...
def link_check(request):
    ...
    try:
        ...
        with open('debug_log.txt', 'a') as f:
            f.write(f"DEBUG: link_check called. User: {user.email if user else 'anonymous'}\n")
    except Exception as e:
        with open('debug_log.txt', 'a') as f:
            f.write(f"DEBUG: link_check auth error: {e}\n")
    ...
    if user and result.get("is_valid"):
        try:
            ...
            with open('debug_log.txt', 'a') as f:
                f.write(f"Scan saved for user {user.username}\n")
        except Exception as e:
            with open('debug_log.txt', 'a') as f:
                import traceback
                f.write(f"Scan save error: {e}\n{traceback.format_exc()}\n")
...
def twilio_webhook(request):
    ...
    is_url = msg_body.lower().startswith('http')
    resp_text = ""
    
    if is_url:
        result = link_validate_url(msg_body)
        risk = result.get("risk_level", "UNKNOWN")
        score = result.get("risk_score", 0)
        flags = result.get("red_flags", [])
        
        emoji = "✅" if risk == "SAFE" else ("⚠️" if risk == "SUSPICIOUS" else "🚨")
        resp_text = f"{emoji} *Link Analysis Result*\n" \
                    f"*Risk:* {risk}\n" \
                    f"*Score:* {score}/100\n" \
        ...
        _send_to_mongo_async('urls', {
            "url": msg_body,
            "riskLevel": risk,
            "user": f"twilio-{sender}", 
            "timestamp": timezone.now().isoformat()
        })
    else:
        result = analyze_message(msg_body)
        risk = RISK_LEVEL_MAP.get(result["risk_level"], "SAFE")
        score = result["scam_score"]
        
        emoji = "✅" if risk == "SAFE" else ("⚠️" if risk == "SUSPICIOUS" else "🚨")
        resp_text = f"{emoji} *Message Analysis Result*\n" \
                    f"*Risk:* {risk}\n" \
                    f"*Score:* {score}/100\n" \
                    f"*Scan Type:* {result.get('scam_type', 'Unknown')}\n" \
                    f"*Advice:* {(result.get('safety_tips', ['Stay cautious']) or ['Stay cautious'])[0]}"
    ...
    <Response>
        <Message>{escape(resp_text)}</Message>
    </Response>"""
