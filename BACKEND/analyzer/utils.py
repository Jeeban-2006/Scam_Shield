...
MONGO_SERVICE_URL = "http://localhost:5000/api"

def _post_mongo_task(endpoint, data):
    try:
        requests.post(f"{MONGO_SERVICE_URL}/{endpoint}", json=data, timeout=3)
    except Exception as e:
        print(f"Mongo Service Error ({endpoint}): {e}")

def _send_to_mongo_async(endpoint, data):
    threading.Thread(target=_post_mongo_task, args=(endpoint, data), daemon=True).start()

def _send_user_to_mongo(user, google_uid=None):
    data = {
        "email": user.email,
    ...
    _send_to_mongo_async("users", data)
