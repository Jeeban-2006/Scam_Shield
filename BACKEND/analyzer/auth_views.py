...
def get_user_from_request(request):
    ...
    try:
        ...
        # Token lookup failed (invalid or too long), fall back to JWT check
        with open('auth_debug.txt', 'a') as f:
            f.write(f"DEBUG: Token ID lookup failed: {e}. Trying JWT.\n")
        try:
            import jwt
            # WARNING: Signature verification skipped. In production, verify with Firebase Admin SDK.
            payload = jwt.decode(key, options={"verify_signature": False})
            with open('auth_debug.txt', 'a') as f:
                f.write(f"DEBUG: JWT Decoded. Email: {payload.get('email')}\n")
            ...
            if email:
                ...
                if not user:
                    with open('auth_debug.txt', 'a') as f:
                        f.write(f"DEBUG: Creating new user for {email}\n")
                    # Auto-provision user from Firebase token data
                    username = payload.get("name") or payload.get("user_id") or email.split("@")[0]
                ...
                # Sync user to MongoDB (store credentials/profile)
                try:
                    from .utils import _send_user_to_mongo
                    _send_user_to_mongo(user, google_uid=payload.get("user_id"))
                except Exception as mongo_err:
                     with open('auth_debug.txt', 'a') as f:
                        f.write(f"DEBUG: Mongo sync failed: {mongo_err}\n")
                
                return user
            else:
                 with open('auth_debug.txt', 'a') as f:
                    f.write("DEBUG: No email in JWT payload\n")
        except Exception as jwt_error:
            with open('auth_debug.txt', 'a') as f:
                f.write(f"DEBUG: JWT Decode failed: {jwt_error}\n")
        return None
...
def login(request):
    ...
    if not User.objects.filter(username__iexact=username).exists():
        return Response({"error": "User not registered. Please sign up first."}, status=404)
