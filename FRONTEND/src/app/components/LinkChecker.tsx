...
export function LinkChecker() {
  ...
  useEffect(() => {
    ...
    import('@/app/lib/localHistory').then(({ getLocalLinkHistory }) => {
      ...
      if (localHistory.length > 0) {
        setRecentScans(
          localHistory.slice(0, 5).map((scan) => ({
            url: scan.url,
            status: riskLevelToStatus(scan.risk_level),
            time: formatRelativeTime(scan.timestamp),
          }))
        );
      }
    });
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      const { saveLocalLinkScan } = await import('@/app/lib/localHistory');
      saveLocalLinkScan(url.trim(), result);
