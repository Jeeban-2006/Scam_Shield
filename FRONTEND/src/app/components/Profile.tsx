...
import { User, Mail, Calendar, Shield, MessageSquare, Link as LinkIcon, FileText, Settings, BookOpen, Phone, Trash2, Eye, EyeOff, AlertTriangle } from 'lucide-react';
import { clearStoredAuth, getStoredAuth, changePassword } from '@/app/api/auth';
import { Avatar, AvatarFallback } from '@/app/components/ui/avatar';
import { getProfile, getStats, getMessageHistory, getLinkHistory, updateProfile, deleteAccount, getMyReports, type ReportItem } from '@/app/api/user';
...
export function Profile() {
  ...
  const [reportActivity, setReportActivity] = useState<ReportItem[]>([]);
  ...
  useEffect(() => {
    ...
    // Load localStorage history immediately for instant display
    import('@/app/lib/localHistory').then(({ getLocalLinkHistory }) => {
      const localHistory = getLocalLinkHistory();
      if (localHistory.length > 0) {
        console.log('📦 Loaded from localStorage:', localHistory.length, 'links');
        const localScans = localHistory.map(scan => ({
          id: parseInt(scan.id),
          content: scan.url,
        ...
        setLinkActivity(localScans);
      }
    });

    Promise.all([getProfile(), getStats(), getMessageHistory(1, 10), getLinkHistory(1, 10), getQuizHistory(1, 10), getMyReports(1, 10)])
      .then(([p, s, msgH, linkH, q, reports]) => {
        console.log('📊 Profile data loaded:', {
          profile: p,
          stats: s,
          messages: msgH?.scans?.length || 0,
          links: linkH?.scans?.length || 0,
          quizzes: q?.attempts?.length || 0,
          reports: reports?.reports?.length || 0,
          reportsData: reports?.reports
        });
        console.log('🔗 Link History Raw Data:', linkH);
        console.log('📧 Message History Raw Data:', msgH);
        setProfile(p);
        setStats(s);
        setMessageActivity(msgH?.scans ?? []);
        // Merge backend data with localStorage if backend has data
        if (linkH?.scans && linkH.scans.length > 0) {
          setLinkActivity(linkH.scans);
        }
        setQuizAttempts(q?.attempts ?? []);
        setReportActivity(reports?.reports ?? []);
      })
      .catch((error) => {
  ...
  const messageItems = messageActivity.map((s) => ({
    ...
    riskLevel: s.risk_level === 'High Risk Scam' || s.risk_level === 'LIKELY_SCAM' ? 'high' : s.risk_level === 'Suspicious' || s.risk_level === 'SUSPICIOUS' ? 'medium' : 'safe',
    sortAt: s.created_at,
  }));
  const linkItems = linkActivity.map((s) => ({
    ...
    result: s.risk_level,
    time: new Date(s.created_at).toLocaleDateString(),
    riskLevel: s.risk_level === 'High Risk Scam' || s.risk_level === 'DANGEROUS' ? 'high' : s.risk_level === 'Suspicious' || s.risk_level === 'SUSPICIOUS' ? 'medium' : 'safe',
    sortAt: s.created_at,
  }));
  ...
  const reportItems = reportActivity.map((r) => ({
    type: 'report' as const,
    title: `Report: ${r.scam_type} (${r.platform})`,
    result: r.status,
    time: new Date(r.created_at).toLocaleDateString(),
    riskLevel: r.status === 'pending' ? 'medium' : 'safe',
    sortAt: r.created_at,
  }));
  const allActivity = [...messageItems, ...linkItems, ...quizItems, ...reportItems].sort(
            ...
            <div className="lg:col-span-2">
              ...
              <div className="mt-8 p-6 sm:p-8 rounded-2xl bg-[#1a1f3a]/50 border border-[#00d9ff]/20">
                <h2 className="text-xl sm:text-2xl font-bold text-white mb-4 sm:mb-6 flex items-center gap-3">
                  <LinkIcon className="w-6 h-6 text-[#00d9ff]" />
                  Link Scan History
                </h2>
                <div className="space-y-3 max-h-[20rem] overflow-y-auto pr-1 scrollbar-theme">
                  {linkActivity.length > 0
                    ? linkActivity.map((link, i) => (
                      <div
                        key={i}
                        className="p-4 rounded-xl bg-[#0a0e27]/50 border border-[#00d9ff]/10 flex items-center justify-between gap-2"
                      >
                        <div className="flex-1 min-w-0">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${link.risk_level === 'DANGEROUS' || link.risk_level === 'LIKELY_SCAM' ? 'bg-red-500/20 text-red-500' :
                          link.risk_level === 'SUSPICIOUS' ? 'bg-yellow-500/20 text-yellow-500' :
                    ...
                    : <p className="text-gray-400">No links scanned yet</p>}
                </div>
              </div>
                  ...
                  {reportActivity.length > 0
                    ? reportActivity.map((report) => (
                      <div
                        ...
                        className="p-4 rounded-xl bg-[#0a0e27]/50 border border-[#00d9ff]/10 hover:border-[#00d9ff]/30 transition-all"
                      >
                        <div className="flex items-start justify-between gap-3 mb-2">
                          ...
                          <div className="flex flex-col items-end gap-1">
                            <span className={`text-xs px-2 py-0.5 rounded ${report.status === 'pending'
